...
for target_code in target_code_lst:
    ...
    cph.fit(mydf, duration_col='Y_yrs', event_col='Y', formula='DM_AGE + DM_GENDER + ProRS')
    pval_lst.append(cph.summary.p.ProRS)
    hr_lst.append(cph.hazard_ratios_.ProRS)
    lbd_lst.append(np.exp(cph.confidence_intervals_.iloc[2, 0]))
    ubd_lst.append(np.exp(cph.confidence_intervals_.iloc[2, 1]))
...
myout_df.to_csv(outpath1 + 'ProRS_HR_AgeSex.csv')
