...
myformula = 'DM_AGE + C(DM_GENDER) + C(DM_ETH) + DM_TDI + DM_EDUC + '\
...
for target_code in target_code_lst:
    ...
    lbd_lst.append(np.exp(cph.confidence_intervals_.iloc[-1, 0]))
    ubd_lst.append(np.exp(cph.confidence_intervals_.iloc[-1, 1]))
...
myout_df.to_csv(outpath1 + 'ProRS_HR_PANEL.csv')
