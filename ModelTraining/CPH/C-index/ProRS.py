...
for target_code in target_code_lst:
    ...
    for fold_id in fold_id_lst:
        mydf_train = pd.read_csv(outpath + 'MLModeling/MLP/ResMLP/' + target_code + '/Train' +str(fold_id) +'_ProRS.csv')
        mydf_test = pd.read_csv(outpath + 'MLModeling/MLP/ResMLP/' + target_code + '/Test' +str(fold_id) +'_ProRS.csv')
        cph = CoxPHFitter(penalizer = 0.01)
        cph.fit(mydf_train, duration_col='Y_yrs', event_col='Y', formula='ProRS')
        try:
    ...
    out_df.to_csv(outpath + 'MLModeling/CPH/ProRS/ProRS_Cidx.csv', index=False)
