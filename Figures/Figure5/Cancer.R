...
mydata = read.csv('C0_PLOT_Data1000.csv')
...
#mydata$shap_values[mydata$shap_values>0.3] = 0.3
#mydata$shap_values[mydata$shap_values< -0.4] = -0.4
...
top_data = mydata1[mydata1$Pro_code %in% c('CXCL14', 'GDF15'), ]
medium_data = mydata1[!(mydata1$Pro_code %in% c('CXCL14', 'GDF15')), ]
...
p = ggplot(mydata, aes(x = Pro_code, y = shap_values, color=Pro_values)) + 
  ...
  geom_text(data=top_data, aes(x=id, y = 0.38, label=Pro_code, hjust=hjust), color="black", size=7.5, fontface = 'bold', angle= top_data$angle, check_overlap = T)+
  geom_text(data=medium_data, aes(x=id, y = 0.38, label=Pro_code, hjust=hjust), color="black", size=7.5, fontface = 'bold', angle= medium_data$angle, check_overlap = T)+
...
#ggsave('CAN.png', width = 17.6, height = 17, device='png', dpi=150)
