...
code_f_lst = ['40001-0.0','40001-1.0']

death_df = pd.read_csv(dpath + 'ukb672277_death_data.csv', usecols = ['eid', '40000-0.0', '40001-0.0', '40001-1.0'])
...
death_df.rename(columns = {'40001-0.0':'target_death', '40000-0.0':'target_date', 'BL2Death_yrs': 'BL2Target_yrs_raw'}, inplace = True)
death_df['target_y'] = 1
death_df['target_y'][death_df['target_date'].isnull() == True] = 0
tmpdf = death_df[['BL2Target_yrs_raw', 'BL2End_yrs']]
death_df['BL2Target_yrs'] = pd.DataFrame([tmpdf.iloc[i,:].min() for i in range(len(death_df))])
target_df_out = death_df[['eid', 'target_death', 'target_date', 'BL2Target_yrs_raw', 'target_y', 'BL2Target_yrs']]
target_df_out.to_csv(outpath + target_code + '.csv', index = False)

################ Cancer     ##################################################
################ ICD10: C00-C97, D00-D48        ##################################################
target_code = 'X1'
target_death_f = ['C0'+str(i) for i in range(10)] + ['C'+str(i) for i in range(10, 97)] + \
                  ['D0'+str(i) for i in range(10)] + ['D'+str(i) for i in range(10, 49)]

################ Nervous System         ##################################################
################ ICD10: G00-G99            ##################################################
target_code = 'X2'
target_death_f = ['G0'+str(i) for i in range(10)] + ['G'+str(i) for i in range(10, 100)]

################ Circulatory system           ##################################################
################ ICD10: I00-I99            ##################################################
target_code = 'X3'
target_death_f = ['I0'+str(i) for i in range(10)] + ['I'+str(i) for i in range(10, 100)]

################ Respiratory system              ##################################################
################ ICD10: J00-J99        ##################################################
target_code = 'X4'
target_death_f = ['J0'+str(i) for i in range(10)] + ['J'+str(i) for i in range(10, 100)]
'''
